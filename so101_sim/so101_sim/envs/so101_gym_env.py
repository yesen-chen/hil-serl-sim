...
_DEFAULT_ACTION_SCALE = np.asarray([0.05, 0.05, 0.05, 0.05, 0.05, 0.02])
...
class So101GymEnv(MujocoGymEnv):
    ...
    def step(
        ...
        return obs, rew, terminated, truncated, {"succeed": False}
    ...
    def _compute_observation(self) -> dict:
        ...
        obs["state"] = {
            ...
            "joint_pos": self._data.qpos[self._qpos_indices].astype(np.float32),
        }
...
if __name__ == "__main__":
    env = So101GymEnv(render_mode="human", image_obs=True)
    env.reset()
    for _ in range(200):
